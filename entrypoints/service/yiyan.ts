...
async function yiyan(message: any) {
	...
	if (resp.ok) {
		...
		if (result.error_code)
			throw new Error(`翻译失败: ${result.error_code} ${result.error_msg}`);
...
async function getSecret() {
	const cached = config.extra[services.yiyan] as
		| { secret?: string; expiration?: number }
		| undefined;
	const secret: string | undefined = cached?.secret;
	...
	if (resp.ok && res.access_token) {
		...
		const nextExpiration = Date.now() + res.expires_in * 1000;
		// 缓存 secret 和 expiration
		config.extra[services.yiyan] = {
