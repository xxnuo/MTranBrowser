...
// 生成腾讯云API签名
async function createTencentSignature(
...
async function tencent(message: any) {
	try {
		...
		if (!secretId || !secretKey) {
			throw new Error(
				"腾讯云机器翻译密钥未配置，请在设置中配置SecretId和SecretKey",
			);
		}
		...
		// 生成签名和Authorization头
		const authorization = await createTencentSignature(
			requestBody,
			timestamp,
			secretId,
			secretKey,
		...
		// 判断是否使用代理
		const url =
			config.proxy[config.service] || "https://tmt.tencentcloudapi.com/";
		...
		if (!response.ok) {
			...
			throw new Error(
				`腾讯云机器翻译请求失败: ${response.status} ${response.statusText}\n${errorText}`,
			);
		}
		...
		if (result.Response?.Error) {
			throw new Error(
				`腾讯云机器翻译错误: ${result.Response.Error.Code} - ${result.Response.Error.Message}`,
			);
		}
