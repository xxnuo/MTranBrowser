...
async function deeplx(message: any) {
	...
	const url: string = config.proxy[config.service]
		...
		: config.deeplx || "http://localhost:1188/translate";
	...
	if (
		config.token[services.deeplx] &&
		config.token[services.deeplx].trim() !== ""
	) {
	...
	if (resp.ok) {
		...
		throw new Error(
			`DeepLX 翻译失败: ${resp.status} ${resp.statusText} body: ${await resp.text()}`,
		);
	}
}
