...
// 生成腾讯云API签名
async function createHunyuanSignature(
	...
	const hashedCanonicalRequestHex = Array.from(
		new Uint8Array(hashedCanonicalRequest),
	)
		.map((b) => b.toString(16).padStart(2, "0"))
		.join("");
...
async function hunyuanTranslation(message: any) {
	try {
		...
		if (!secretId || !secretKey) {
			throw new Error(
				"腾讯混元翻译密钥未配置，请在设置中配置SecretId和SecretKey",
			);
		}
		...
		if (secretId.length < 10 || secretKey.length < 10) {
			throw new Error(
				"SecretId或SecretKey格式不正确，请检查是否完整复制了密钥信息",
			);
		}

		// 转换语言代码
		// 对于自动检测，使用MTranBrowser内置的语言检测
		let sourceLang: string;
		if (config.from === "auto") {
			const detectedLang = detectlang(
				message.origin.replace(/[\s\u3000]/g, ""),
			);
		...
		// 生成签名和Authorization头
		const authorization = await createHunyuanSignature(
			requestBodyStr,
			timestamp,
			secretId,
			secretKey,
		...
		// 判断是否使用代理
		const url =
			config.proxy[config.service] || "https://hunyuan.tencentcloudapi.com/";
		...
		if (!response.ok) {
			...
			throw new Error(
				`腾讯混元翻译请求失败: ${response.status} ${response.statusText}\n${errorText}`,
			);
		}
		...
		if (result.Response?.Error) {
			...
			throw new Error(
				`腾讯混元翻译错误: ${result.Response.Error.Code} - ${result.Response.Error.Message}`,
			);
		}
