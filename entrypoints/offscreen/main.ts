...
function isChromeTranslationSupported(): boolean {
	...
	if (
		"translation" in self &&
		"createTranslator" in (self as any).translation
	) {
		return true;
	}
...
async function detectLanguage(text: string): Promise<string> {
	try {
		...
		if (
			"translation" in self &&
			"createDetector" in (self as any).translation
		) {
...
async function handleTranslationRequest(data: any): Promise<string> {
	...
	if (!isChromeTranslationSupported()) {
		throw new Error(
			"当前浏览器不支持 Chrome Translation API，请确保使用 Google Chrome 浏览器 v138 stable 或更高版本。",
		);
	}
	...
	try {
		...
		if (error instanceof Error) {
			if (
				...
				throw new Error(
					"Chrome Translation API 暂时不可用。可能需要下载语言模型，请稍后重试。",
				);
			} else if (
				...
				throw new Error(
					`不支持的语言组合：${fromLang} -> ${toLang}。请尝试其他语言对或检查浏览器版本。`,
				);
