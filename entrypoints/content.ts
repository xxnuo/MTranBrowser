...
export default defineContentScript({
	...
	async main() {
		...
		// 处理悬浮球控制消息
		browser.runtime.onMessage.addListener(
			(message: any, _sender: any, sendResponse: () => void) => {
...
function setupManualTranslationTriggers() {
	...
	const checkMouseHotkey = () => {
		...
		const allKeysPressed = hotkeyParts.every((key) =>
			mouseHotkeysPressed.has(key),
		);
		const exactMatch =
			allKeysPressed && hotkeyParts.length === mouseHotkeysPressed.size;
	...
	window.addEventListener("keyup", (event) => {
		...
		if (releasedCode?.startsWith("key")) {
		...
		// 获取当前配置的快捷键
		const _hotkeyParts = getConfiguredMouseHotkeyParts();
	...
	document.body.addEventListener("touchstart", (event) => {
		...
		switch (config.hotkey) {
			case constants.TwoFinger:
				coordinate = getCenterPoint(event.touches, 2) || null;
				break;
			case constants.ThreeFinger:
				coordinate = getCenterPoint(event.touches, 3) || null;
				break;
			case constants.FourFinger:
				coordinate = getCenterPoint(event.touches, 4) || null;
				break;
			default:
				return;
		}
	...
	let timer: number;
	const startPos = { x: 0, y: 0 }; // startPos 记录鼠标按下时的位置
	...
	document.body.addEventListener("mousemove", (event) => {
		// 如果鼠标移动超过10像素，取消长按事件
		if (
			Math.abs(event.clientX - startPos.x) > 10 ||
			Math.abs(event.clientY - startPos.y) > 10
		) {
			clearTimeout(timer);
		}
	});
	document.body.addEventListener("mousemove", (event) => {
		...
		if (
			(config.hotkey === constants.LongPress &&
				Math.abs(event.clientX - startPos.x) > 10) ||
			Math.abs(event.clientY - startPos.y) > 10
		) {
			clearTimeout(timer);
	...
	document.body.addEventListener("touchstart", (event) => {
		...
		if (
			![constants.DoubleClickScreen, constants.TripleClickScreen].includes(
				config.hotkey,
			) ||
		...
		// 确定需要的点击次数
		const requiredTouches =
			config.hotkey === constants.DoubleClickScreen ? 2 : 3;
...
function setupFloatingBallHotkey() {
	...
	if (isDev) {
		console.log(
			`[MTranBrowser] 设置悬浮球快捷键: ${config.floatingBallHotkey}, 系统: ${isMac ? "macOS" : "其他"}`,
		);
	}
	...
	document.addEventListener("keydown", (event) => {
		...
		const exactMatch =
			allKeysPressed && hotkeyParts.length === hotkeysPressed.size;
			...
			if (isDev) {
				const activeHotkey =
					config.floatingBallHotkey === "custom"
						? config.customFloatingBallHotkey
						: config.floatingBallHotkey;
				console.log(`[MTranBrowser] 触发悬浮球翻译，快捷键: ${activeHotkey}`);
			}
		}
	});
...
// 清除所有翻译的函数
function _clearAllTranslations() {
	// 1. 移除所有翻译结果元素
	document.querySelectorAll(".fluent-read-translation").forEach((el) => {
		el.remove();
	});

	// 2. 移除所有加载状态
	document.querySelectorAll(".fluent-read-loading").forEach((el) => {
		el.remove();
	});

	// 3. 移除所有错误状态
	document.querySelectorAll(".fluent-read-failure").forEach((el) => {
		el.remove();
	});
...
function setupInputBoxTranslation() {
	...
	document.addEventListener("keydown", async (event) => {
		...
		if (triggerType === "ctrl_enter") {
			...
			triggerType === "triple_space" ||
			triggerType === "triple_equal" ||
			triggerType === "triple_dash"
		) {
			// 连按三次触发
			let targetKey = "";
			switch (triggerType) {
...
function createTranslationTooltip(
	...
	tooltip.dir = getLanguageDirection(resolveUiLanguage(config.uiLanguage));
...
async function translateWithMicrosoft(
	...
	try {
		...
		if (result?.success) {
			...
			throw new Error(result?.error || t("微软翻译失败"));
		}
	} catch (error) {
...
async function handleInputBoxTranslation(element: HTMLElement): Promise<void> {
	...
	try {
		...
		// 根据触发方式去除末尾的触发符号
		const cleanedText = removeTriggerSymbols(
			originalText,
			config.inputBoxTranslationTrigger,
		);
		...
		_tooltip = createTranslationTooltip(
			element,
			t("微软翻译中"),
			"translating",
		);
		...
		try {
			...
			const translatedText = await translateWithMicrosoft(
				cleanedText,
				config.inputBoxTranslationTarget,
			);
			...
			if (translatedText && translatedText !== cleanedText) {
				...
				removeExistingTooltip();
				_tooltip = createTranslationTooltip(element, t("翻译成功"), "success");
			} else {
				// 翻译结果与原文相同或为空
				element.classList.remove("fluent-input-translating");
			...
			addInputBoxAnimation(element, "error");
			removeExistingTooltip();
			_tooltip = createTranslationTooltip(element, t("微软翻译失败"), "error");
