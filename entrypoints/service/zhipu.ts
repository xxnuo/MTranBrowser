...
async function zhipu(message: any) {
	...
	const token = config.token[services.zhipu];
	const cached = config.extra[services.zhipu] as
		| { secret?: string; expiration?: number }
		| undefined;
	let secret: string | undefined = cached?.secret;
	const expiration: number | undefined = cached?.expiration;
	if (!secret || typeof expiration !== "number" || expiration <= Date.now()) {
		secret = generateToken(token);
		if (!secret) throw new Error("无法生成令牌");
		// 保存 secret 和 expiration
		config.extra[services.zhipu] = {
			secret,
...
function generateToken(APIKey: string) {
	...
	const duration = 3600000 * 24; // 生成的 token 默认24小时后过期
...
function generateJWT(secret: string, header: any, payload: any) {
	...
	// 生成 jwt 签名
	const hmacsha256 = base64UrlSafe(
		CryptoJS.HmacSHA256(`${encodedHeader}.${encodedPayload}`, secret).toString(
			CryptoJS.enc.Base64,
		),
	);
