...
async function azureOpenai(message: any) {
	try {
		...
		if (!apiKey || apiKey.trim() === "") {
			throw new Error(
				"Azure OpenAI API Key 未配置，请在设置中输入有效的 API Key",
			);
		}
		...
		if (!endpoint || endpoint.trim() === "") {
			throw new Error(
				"Azure OpenAI 端点地址未配置，请在设置中输入完整的端点地址",
			);
		}
		...
		if (
			...
			throw new Error(
				"Azure OpenAI 端点地址格式不正确，请确保包含正确的域名和路径",
			);
		}
		...
		if (!resp.ok) {
			...
			switch (resp.status) {
				case 401:
					errorMessage =
						"API Key 无效或已过期，请检查您的 Azure OpenAI API Key";
					break;
				case 404:
