...
function buildKey(message: string) {
	...
	const selectedModel =
		model[service] === customModelString
			? customModel[service]
			: model[service];
	const reasoningEffort = servicesType.isOpenAICompatible(service)
		? config.reasoningEffort?.[service] || "default"
		: "";
	const extraParamsHash = servicesType.isOpenAICompatible(service)
		? hashText((config.openaiExtraParams?.[service] || "").trim())
		: "";
	return [
		prefix,
		style,
...
export const cache = {
	...
	cleaner() {
		...
		if (
			...
			currentTime - parseInt(lastSessionTimestamp, 10) > 24 * 3600000
		) {
			this.clean();
...
export function stringifyNode(node: any): string {
	...
	const outerHTML = serializer.serializeToString(node);
	// 移除多余的空白符
