...
export function grabNode(node: any): any {
	...
	// 2. 特殊适配：根据域名进行特殊处理
	const domainHandler =
		selectCompatFn[getMainDomain(location.href.split("?")[0])];
	if (domainHandler) {
...
function isMainlyNumericContent(node: any): boolean {
	...
	// 如果所有文本节点都是数字，则跳过翻译
	// 这可能是表格中的数字列或者纯数字列表等
	if (textNodes.length > 0 && textNodes.every((t) => isNumericContent(t)))
		return true;
...
function isUserIdentifier(text: string): boolean {
	...
	// 检查是否包含"关注"相关内容
	if (/关注.*\w+/.test(trimmedText) || /Follow.*\w+/.test(trimmedText))
		return true;
...
function isNumericContent(text: string): boolean {
	...
	if (
		/^(\d{4}[-/]\d{1,2}[-/]\d{1,2}|\d{1,2}[-/]\d{1,2}[-/]\d{4}|\d{1,2}[-/]\d{1,2}[-/]\d{1,2})$/.test(
			trimmedText,
		)
	)
		return true;
...
function detectChildMeta(parent: any): boolean {
	...
	while (child) {
		if (
			...
			!inlineSet.has(child.nodeName.toLowerCase())
		) {
			return false;
		}
		child = child.nextSibling;
