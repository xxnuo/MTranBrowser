...
import { createTranslator, localizeOptions } from "@/lib/i18n";
import {
...
function getErrorMessage(errMsg: string): string {
	...
	if (
		...
		errMsg.includes("令牌") ||
		errMsg.includes("SecretId") ||
		errMsg.includes("SecretKey")
	) {
		return t("Token 似乎有点问题，请前往设置页面重新配置后再试。");
	} else if (
		...
		const service = localizeOptions(options, t).services.find(
			(s: { value: string; label: string }) => s.value === config.service,
		);
		return t("你的请求频率过高，被【{service}】拒绝了，请稍后再试吧~", {
			service: service?.label || config.service,
		});
	} else if (
		...
		return t("网络连接好像不稳定，请检查网络后再试。");
	} else if (normalized.includes("model") || errMsg.includes("模型")) {
		return t("模型配置可能有误，请前往设置页面进行检查和调整。");
	} else if (normalized.includes("timeout") || errMsg.includes("超时")) {
		return t("请求超时啦，请稍后再试一次。");
	} else {
		return errMsg || t("出现了未知错误，请前往开源社区联系开发者吧~");
	}
}
...
export function insertLoadingSpinner(
	...
	if (isCache) spinner.style.borderTop = "2px solid #6b7280";
